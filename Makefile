...
NUTERM = nuterm
NUTERM_DIR = thirdparty/$(NUTERM)
NUTERM_AR = $(NUTERM_DIR)/lib$(NUTERM).a
NUTERM_FLAT = build/$(NUTERM)

THIRDPARTY_CFLAGS = -I$(NUTERM_DIR)/include
...
$(LIB_AR_FILE): $(C_OBJ) $(NUTERM_FLAT)
	$(AR) rcs $@ $(C_OBJ) $(NUTERM_FLAT)/*.o

$(LIB_SO_FILE): $(C_OBJ) $(NUTERM_FLAT)
	$(CC) -shared $(C_OBJ) -o $@ $(NUTERM_FLAT)/*.o

$(NUTERM_FLAT): $(NUTERM_AR)
	@mkdir -p $(NUTERM_FLAT)
	$(AR) x $(NUTERM_AR) --output=$(NUTERM_FLAT)

$(NUTERM_AR): $(NUTERM_DIR)
	make -C $(NUTERM_DIR) LIB_TYPE=ar OPT=2
